...
url="https://www.ccreadbible.org/chinesebible/znsigao"
...
printf -v output "${temp}/OEBPS/%02d_%s.xhtml" ${b} ${scbn}
echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" > ${output}
echo "<html xmlns=\"http://www.w3.org/1999/xhtml\">" >> ${output}
echo "<head>" >> ${output}
echo "<title>${scbn}</title>" >> ${output}
echo "</head>" >> ${output}
echo "<body>" >> ${output}
echo "<h1>${scbn}</h1>" >> ${output}
echo "</body>" >> ${output}
echo "</html>" >> ${output}
...
printf -v output "${temp}/OEBPS/%02d_%s_%d.xhtml" ${b} ${scbn} ${i}
echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" > ${output}
echo "<html xmlns=\"http://www.w3.org/1999/xhtml\">" >> ${output}
echo "<head>" >> ${output}
echo "<title>${scbn} : Chapter ${i}</title>" >> ${output}
echo "</head>" >> ${output}
echo "<body>" >> ${output}
echo "<h1>${scbn} : Chapter ${i}</h1>" >> ${output}
...
echo "        <dc:title>思高圣经</dc:title>" >> ${temp}/OEBPS/content.opf
...
p=$(pwd)
cd ${temp}
zip -X0 ${p}/思高圣经.epub mimetype
zip -Xr9D ${p}/思高圣经.epub META-INF OEBPS
